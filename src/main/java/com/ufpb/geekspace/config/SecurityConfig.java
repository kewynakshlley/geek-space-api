...
public class SecurityConfig extends WebSecurityConfigurerAdapter implements WebMvcConfigurer {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable().authorizeRequests().antMatchers(HttpMethod.POST, "/login").permitAll()
				...
				.antMatchers(HttpMethod.GET, "/all-product/category-filter/*").permitAll()
				.antMatchers(HttpMethod.GET, "/shirt-product/color-filter/*").permitAll()
				.antMatchers(HttpMethod.GET, "/shirt-product/color-genre-filter/*").permitAll()
				.antMatchers(HttpMethod.GET, "/shirt-product/genre-filter/*").permitAll()
